...
function resolveModelCosts(map, entry, model) {
  ...
  return familyFallback(map, name);
}

// A model we've never seen — `claude-opus-6` the week it ships, before LiteLLM
// carries it. Rather than bill it $0 (a silent undercount that looks like a real
// number), inherit the newest known entry in the same tier: Anthropic has held
// per-tier pricing steady across generations, so it is usually right and always
// closer than zero. Flagged `approximate` so every consumer can mark it — an
// unmarked plausible guess is harder to catch than an obvious zero.
// Returns a COPY: the map's own entries must stay unflagged and shared.
function familyFallback(map, name) {
  const m = /^claude-([a-z]+)-\d/.exec(name);
  if (!m) return null;
  const family = m[1];
  const re = new RegExp(`^claude-${family}-(\\d+)(?:-(\\d+))?$`);
  let best = null, bestVer = [-1, -1];
  for (const k of Object.keys(map)) {
    const km = re.exec(k);
    if (!km) continue;
    const ver = [Number(km[1]), km[2] ? Number(km[2]) : 0];
    if (ver[0] > bestVer[0] || (ver[0] === bestVer[0] && ver[1] > bestVer[1])) { best = map[k]; bestVer = ver; }
  }
  return best ? { ...best, approximate: true } : null;
}
...
// Ask for a refresh, subject to the 1h attempt throttle (a stamp file, so a
// persistently failing fetch can't fire on every prompt). `staleAfter` is the
// timestamp before which we don't even consider fetching — loadPricing passes
// fetchedAt + 24h; a caller that has just seen an unknown model passes 0 to skip
// the TTL and go now. The throttle still applies, so a model LiteLLM will never
// carry (a Bedrock ARN, a private deployment) costs at most one request an hour.
function requestFetch(stateDir, opts = {}) {
  // The explicit stay-offline switch wins over every caller, including the
  // unknown-model path — an opted-out user must never hit the network.
  if (process.env.STATUSLINE_PRICING_NO_FETCH) return false;
  const staleAfter = opts.staleAfter || 0;
  if (Date.now() < staleAfter) return false;
  const stamp = path.join(stateDir, 'pricing.last-attempt');
  let lastAttempt = 0;
  try { lastAttempt = fs.statSync(stamp).mtimeMs; } catch {}
  if (Date.now() - lastAttempt <= FETCH_RETRY_MS) return false;
  try {
    fs.mkdirSync(stateDir, { recursive: true });
...
function loadPricing(stateDir, opts = {}) {
  ...
  if (allowFetch) requestFetch(stateDir, { staleAfter: fetchedAt + FETCH_TTL_MS });
  return { map, pricingHash: hashMap(map) };
}

module.exports = { sanitizeRate, requestFetch, isLocalModel, buildMap, isUsablePriceTable, getModelCosts, hashMap, loadPricing };
