...
function buildMap(rawObj) {
  ...
  for (const [name, e] of Object.entries(rawObj || {})) {
    ...
    // Fast-mode (speed:'fast') price multiplier. LiteLLM carries it under a
    // provider-specific block; absent for every Claude entry today → default 1.
    const ps = e.provider_specific;
    const fast = ps && typeof ps.fast === 'number' && isFinite(ps.fast) && ps.fast > 0 ? ps.fast : 1;
    const val = {
      input, output,
...
function hashMap(map) {
  ...
  for (const k of Object.keys(map).sort()) {
    ...
    h.update(`${k}:${v.input},${v.output},${v.cacheWrite},${v.cacheRead},${v.fastMultiplier || 1}${big}`);
  }
