...
function calculateCostBreakdown(usage, costs) {
  ...
  const input = inputTokens * rates.input;
  const output = num(usage.output_tokens) * rates.output;
  const cacheWrite = fiveMinute * rates.cacheWrite + oneHour * rates.cacheWrite * 1.6;
  const cacheRead = cacheReadTokens * rates.cacheRead;
