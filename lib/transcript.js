...
// first. Returns [{ id, ts }] with ts = file mtime (unix seconds) — the closest
// available "when" now that the cost ledger (which carried end timestamps) is gone.
// Subagent transcripts live under <id>/subagents/ (nested), so listing only the
// immediate *.jsonl of each project dir excludes them by construction. `dirs`
// (from projectDirs) may be passed to avoid re-listing projects/.
function listSessions(root, dirs) {
  ...
  for (const d of dirs || projectDirs(root)) {
    ...
    for (const e of entries) {
      ...
      try {
        const ts = Math.floor(fs.statSync(path.join(d, e.name)).mtimeMs / 1000);
        out.push({ id: e.name.slice(0, -'.jsonl'.length), ts });
      } catch {}
    }
  }
