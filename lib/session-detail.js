...
const { dayKey, shouldCount, isExpectedNullPrice } = require('./cost-aggregate');
...
// { id, requestId, ts, usage, model, prompt, turn, tools }. within-file: keep LAST
// usage/tools/requestId per message.id, carry FIRST timestamp + FIRST active prompt.
// id-less calls always kept. `requestId` (top-level transcript field, LAST occurrence)
// feeds the same global dedup as lib/cost-aggregate.js so the detail total matches COST. When `trackPrompts`, each call is tagged with the active user prompt and a
...
// Global dedup: shared shouldCount(), files processed oldest mtime first (so the
// total equals lib/cost-aggregate.js's per-session total — same (id,requestId)
// retry-billing). Returns:
...
function buildDetail(mainFile, subagentFiles, pricing) {
  ...
  const seen = new Map(); // id -> Set<requestId>, shared shouldCount() dedup
  ...
  const mainIdx = []; // per kept main call: its index in the main files' combined parse order
  // Per-file turn/step counters restart at 0, but a cross-cwd resume has TWO main
  // halves — offset the second half's indices so its turns don't merge into the
  // first half's rows and its consumer events keep the right steps-remaining.
  let mainTurnOffset = 0;
  for (const d of descriptors) {
    const evBefore = consumerEvents.length;
    const stepOffset = mainParsedSteps;
    ...
    if (d.isMain) {
      ...
      for (let k = evBefore; k < consumerEvents.length; k++) consumerEvents[k].afterStep += stepOffset;
    }
    for (let fi = 0; fi < parsed.length; fi++) {
      const call = parsed[fi];
      const turn = d.isMain ? call.turn + mainTurnOffset : null;
      ...
      if (b.total <= 0) {
        ...
        // But a local/synthetic model's null price is legitimate (isExpectedNullPrice), so
        // exclude it — matching cost-aggregate, else the detail view warns spuriously.
        if (!costs && !isExpectedNullPrice(call.model)) { unpriced++; unpricedModels.add(call.model); }
      ...
      if (d.isMain) {
        ...
        const pp = byPrompt.get(turn) || { turnIndex: turn, text: call.prompt, ts: call.ts || null, cost: 0, calls: 0, inp: 0, ctx: 0, cw: 0, out: 0, peakCtx: 0, tools: new Map() };
        ...
        byPrompt.set(turn, pp);
      } else {
        subagentTotal += b.total; subWithCost.add(d.name);
      }
    }
    // Next main half's turns start above every turn this half produced (+1 so its
    // turn-0 "(session start)" can't collide with this half's last turn).
    if (d.isMain) mainTurnOffset += parsed.reduce((mx, c) => Math.max(mx, c.turn), 0) + 1;
  }
