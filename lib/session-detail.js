...
function parseCalls(file, trackPrompts) {
  ...
  for (const line of raw.split('\n')) {
    ...
    if (!o) continue;
    if (trackPrompts) { const p = promptText(o); if (p) { current = p; turn++; } }
  ...
  for (const d of descriptors) {
    for (const call of parseCalls(d.file, d.isMain)) {
      ...
      if (d.isMain) {
        ...
        const pp = byPrompt.get(call.turn) || { text: call.prompt, ts: call.ts || null, cost: 0, calls: 0, inp: 0, ctx: 0, cw: 0, out: 0, peakCtx: 0, tools: new Map() };
