...
//   topPrompts:[{text,cost,calls,ctx,out,tools:[[name,count]]}],
//   subagentTotal, subagentCount }.
// Per topPrompt: ctx = cache-read tokens re-read across the turn (the dominant
// cost driver), out = output tokens, tools = tool tally (desc by count).
...
function buildDetail(mainFile, subagentFiles, pricing) {
  ...
  for (const d of descriptors) {
    for (const call of parseCalls(d.file, d.isMain)) {
      ...
      if (d.isMain) {
        const pp = byPrompt.get(call.prompt) || { text: call.prompt, cost: 0, calls: 0, ctx: 0, out: 0, tools: new Map() };
        pp.cost += b.total; pp.calls += 1;
        pp.ctx += (call.usage.cache_read_input_tokens || 0); // context re-read this turn (the cost driver)
        pp.out += (call.usage.output_tokens || 0);
  ...
  return {
    ...
    topPrompts: [...byPrompt.values()]
      .map((p) => ({ ...p, tools: [...p.tools.entries()].sort((a, b) => b[1] - a[1]) }))
      .sort(desc),
