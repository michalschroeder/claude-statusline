...
function buildDetail(mainFile, subagentFiles, pricing) {
  ...
  for (const d of descriptors) {
    for (const call of parseCalls(d.file, d.isMain)) {
      ...
      if (d.isMain) {
        const pp = byPrompt.get(call.prompt) || { text: call.prompt, cost: 0, calls: 0, inp: 0, ctx: 0, cw: 0, out: 0, tools: new Map() };
        const cc = extractCacheCreation(call.usage);
        pp.cost += b.total; pp.calls += 1;
        pp.inp += (call.usage.input_tokens || 0);             // fresh (uncached) input
        pp.ctx += (call.usage.cache_read_input_tokens || 0);  // context re-read — the dominant cost driver
        pp.cw += cc.fiveMinute + cc.oneHour;                  // cache-write (new context cached)
